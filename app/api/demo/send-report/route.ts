...
export async function POST(request: NextRequest) {
  try {
    ...
    const { success } = rateLimit(`demo-report:${ip}`, { maxRequests: 3, windowMs: 60_000 })
    if (!success) {
    ...
    const { error } = await resend.emails.send({
      ...
      to: email,
      subject: `JobProof Demo Report${jobId ? ` - ${jobId}` : ''}`,
      html
    })
    ...
    console.error('Demo send report error:', error)
